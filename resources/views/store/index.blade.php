...
@section('content')
  ...
  <div class = "row">
    ...
    <div class="col-md-9">
      ...
      <div class="item"><div class = "product-card"><a href="{{ route('store.single', $item->slug) }}"><img src="{{ Storage::url('images/items/'.'tn_'.$item->picture) }}" alt="thumbnail"/></a><h3><a href="{{ route('store.single', $item->slug) }}">{{$item->title}}</a></h3><br /> <p class = "price">${{$item->price}}</p><br /> <button>Buy Now</button></div></div>
      @endforeach
